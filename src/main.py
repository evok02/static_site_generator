...
def generate_page_recursively(source, template_path, destination):
    ...
    for item in os.listdir(source):
        ...
        if os.path.isdir(current_path):
            ...
            generate_page_recursively(current_path, template_path, new_destination)
        if os.path.isfile(current_path):
            print(f"Creating file in: {current_path}")
            correct_name = item.split(".")[0] + ".html"
            generate_page(current_path, template_path, os.path.join(destination, correct_name))
